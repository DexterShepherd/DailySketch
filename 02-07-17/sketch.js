...
new p5(p => {
  ...
  p.setup = function() {
    ...
    crankA = new Crank(100, 100, 3, 2);
    crankB = new Crank(600, 100, 5, 2);
    render = true;
  }
  ...
  p.draw = function() {
    ...
    p.rotate(speed * 0.05);
    for(let i = 0; i < 200; i++) {
      let theta = p.map(i, 0, 200, -p.PI, p.PI);
      theta = p.sin(theta) + p.cos(theta);
      p.push();
      p.rotate(theta);
      p.translate(crankA.thetaX * crankB.thetaY * theta * 300, crankA.thetaY * crankB.thetaX * theta * 300);
      for(let j = 0; j < 200; j++) {
        p.push();
        theta = p.map(j, 0, 200, -p.PI, p.PI);
        p.rotate(speed * theta);
        p.translate(p.sin(speed + theta) * p.sin(speed) * 100, p.cos(speed + theta) * p.cos(speed) * 100);
        p.point(0, 0);
        p.pop();
      }
      p.pop();
